        ...
        //for (uint8_t amux = 0; amux < AMUX_COUNT; amux++) {
            ...
            //for (uint8_t col = 0; col < amux_n_col_sizes[amux]; col++) {
                ...
                for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
                    ...
                    if (is_unused_position(row, col)) continue;
#endif
                    disable_unused_row(row);
                    //ec_config.noise_floor[row][adjusted_col] += ec_readkey_raw(amux, row, col);
                    ec_config.noise_floor[row][col] += ec_readkey_raw(row, col);
                }
            //}
        //}
        wait_ms(5);
    ...
    //for (uint8_t amux = 0; amux < AMUX_COUNT; amux++) {
        ...
        //for (uint8_t col = 0; col < amux_n_col_sizes[amux]; col++) {
            ...
            for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
                ...
                sw_value[row][adjusted_col] = ec_readkey_raw(row, col);
...
uint16_t ec_readkey_raw(uint8_t row, uint8_t col) {
    uint16_t sw_value = 0;
